...
(() => {
  ...
  var DB_NAME = "sjr-kalananti-web";
  var DB_STORE = "state";
  var DB_KEY = "scratchjr.sqlite";
  ...
  var BrowserInterface = class {
    ...
    io_getmediadata(key, offset, length) {
      const contents = this.mediaCache.has(String(key)) ? this.mediaCache.get(String(key)) : this.io_getmedia(key);
    ...
    resourceAudio(name) {
      ...
      if (!raw) return null;
      const bytes = Uint8Array.from(raw, (character) => character.charCodeAt(0) & 255);
    ...
    async importSQLite(arrayBuffer) {
      ...
      try {
        imported.run("ALTER TABLE PROJECTS ADD COLUMN ISGIFT INTEGER DEFAULT 0");
      } catch (error) {
      }
      this.db = imported;
      this.currentAudio = {};
    ...
    recordsound_recordstop() {
      return this.getAudioCaptureElement().stopRecord();
    }
    recordsound_volume() {
    ...
    recordsound_stopplay() {
      return this.getAudioCaptureElement().stopPlay();
    }
    askForPermission() {
      return true;
    }
  ...
  async function bootstrap() {
    ...
    const SQL = await (0, import_sql.default)({ locateFile: () => SQL_WASM_URL });
    const sql = saved ? new SQL.Database(new Uint8Array(saved)) : new SQL.Database();
    DB_SCHEMA.forEach((statement) => sql.run(statement));
    try {
      sql.run("ALTER TABLE PROJECTS ADD COLUMN ISGIFT INTEGER DEFAULT 0");
    } catch (error) {
    }
    try {
