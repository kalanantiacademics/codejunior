...
const WEB_ASSET_VERSION = 'kalananti-center-layout-20260906';
...
export function preprocessAndLoad (url) {
    ...
    if (window.tablet) {
    	...
    	var xmlhttp = new XMLHttpRequest();
    	 xmlhttp.open('GET', url + (url.indexOf('?') > -1 ? '&' : '?') + 'v=' + WEB_ASSET_VERSION, false);
    	xmlhttp.send();
...
export function preprocessAndLoadCss (baseUrl, url) {
    ...
    cssData = cssData.replace(/url\(\s*(['"]?)([^'")]+)\1\s*\)/g, function (match, quote, path) {
...
function isFrameRoot (el) {
    return el && (el.id == 'frame' || el.id == 'libframe' || el.id == 'paintframe');
}

export function localx (el, gx) {
    var lx = gx;
    while (el && el.offsetTop != undefined && !isFrameRoot(el)) {
...
export function globalx (el) {
    var lx = 0;
    while (el && el.offsetLeft != undefined && !isFrameRoot(el)) {
...
export function css_vh (y) {
    return (y * DESIGN_HEIGHT / 100.0) + 'px';
}

export function css_vw (x) {
    return (x * DESIGN_WIDTH / 100.0) + 'px';
}
