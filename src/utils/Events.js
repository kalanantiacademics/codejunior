...
export default class Events {
    ...
    */
    ...
    static getTargetPoint (e) {
        ...
        // Touch handlers can also receive a synthetic mouse event. Keep the
        // desktop fallback so pointer coordinates never become undefined.
        if (x == undefined || y == undefined) {
            x = e.clientX;
            y = e.clientY;
        }

        // The browser build scales #frame as one logical 1020 x 768 canvas.
        // Convert physical pointer coordinates back into that logical space
        // before the legacy editor hit-tests and drag math use them.
        var activeFrame = (gn('paintframe') && gn('paintframe').classList.contains('appear'))
            ? gn('paintframe')
            : (gn('libframe') && gn('libframe').classList.contains('appear'))
                ? gn('libframe')
                : gn('frame');
        var layoutScale = activeFrame ? Number(activeFrame.dataset.responsiveScale) : 1;
        if (activeFrame && layoutScale > 0) {
            var frameRect = activeFrame.getBoundingClientRect();
            return {
